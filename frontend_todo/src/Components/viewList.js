...
function ViewList({isAuthenticated, setIsAuthenticated}) {
	...
	useEffect(() => {
		const loadData = async () => {
			let response = null;
			try {
				let url = `${BASE_URL}/api/todo/${pageNumber - 1}/${pageSize}`;

				if(filter === 'Completed'){
					url = `${BASE_URL}/api/todo/${pageNumber - 1}/${pageSize}?isCompleted=true`;
				} else if(filter === 'Not Completed'){
					url = `${BASE_URL}/api/todo/${pageNumber - 1}/${pageSize}?isCompleted=false`;
				}
	...
	const markCompleted = async (id) => {
		try {
      await axios.put(`${BASE_URL}/api/todo/${id}/markcomplete`, {}, {
				headers: {
